...
If a modded character exposes no Rare cards for the selected mode, Fool's Gambit falls back to **All Card Pools**. If no Rare cards exist anywhere, the transformation is marked complete and the deck is left unchanged rather than trapping the run in a repeated selection prompt.
